...
| Stays silent, **the vulnerable line did not execute** | a sink in a branch the request did not reach, or downstream of code that threw first, is never observed | a conditional sink not taken (for example `SSRF/task2` when the URL does not match). Note this is narrow: on `VulnerableComponents/attack1` the XStream RCE gadget crashes on JDK 23, yet benign XML still reaches `XStream.fromXML` and IS reported, so "exploit failed" is not the same as "sink never ran." |
